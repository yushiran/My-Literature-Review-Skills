...
- If a previous guide is given as input alongside new abstracts marked
  `[new]`, write a **delta**: keep the previous taxonomy, add each new paper to
  its group with one sentence, extend the timeline, and revise only the gaps
  that the new papers close or open. Do not rewrite unchanged text.
